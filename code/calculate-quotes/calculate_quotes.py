...
words = []
path = ""
depth = 10 # 100 ist einfach zuviel....
efSearch = 16
lang = "skt"
...
class calculate_all:
    ...
    def process_folder(self,filepath,vector_path):
        ...
        def process_file(current_file):
            ...
            filename = re.sub(r'_word.*',r'',filename)
            filename = re.sub(r'_vector.*',r'',filename)
...
def process_folder(path,lang):
    vector_path = re.sub("folder.*","",path)
    tibwords = []
    global tibfilename_list
    tibfilename_list = []
    global tibvector_list
    tibvector_list = []
    if not(os.path.isfile(path + "vectors.idx")):
        for file in os.listdir(path):
            if ".p" in file and not "wordlist" in file:
                ...
                tibfilename_list.append(words_file)
                #print(words_file)
                #print(vector_file)
                current_tibwords = pickle.load(open(path + words_file,'rb'))
                tibvectors = np.load(open(path + vector_file,'rb'))
                #print("TIBVECTORS SHAPE",tibvectors.shape)
                tibwords.extend(current_tibwords)
                if tibvectors.shape[0] == 0:
                    print("TIBFILENAME",file)
                    print("SHAPE",tibvectors.shape)
                else:
...
#process_folder("/mnt/output/tib/tab/folder0/","/mnt/output/tib/tab/",lang)

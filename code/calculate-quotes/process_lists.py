...
folderpath = "/mnt/output/skt/data/folder0/"
lang = "skt"
...
def process_individual_result(result,lang):
    ...
    if result[1]+windowsize < len(words):
        ...
        if result_score < cutoff and words[int(result[1])][0] == words[int(result[1])+windowsize][0]:
            ...
            current_result_position_beg = words[result_position][-1]
            current_result_position_end = words[result_position+windowsize][-1]
            ...
            return [result_position, "\t" + words[result_position][0] + "$" + str(result_position) + "$" + segment +  "$" + str(result_score) +  "$" + str(current_result_position_beg)+  "$" + str(current_result_position_end) + "$" + current_result_sentence.strip().replace("\n"," ")]
        else:
            return [0, ""]
    else:
        return [0, ""]
...
def process_result_npys(path,lang):
    ...
    for file in os.listdir(path):
        ...
        filename_short = re.sub('_result.*','',filename)
        if "results.npy.gz" in filename:
            if not "tab.gz" in filename and not os.path.isfile(path+filename_short + ".tab.gz") and not os.path.isfile(path+filename[:-3] + ".tab"):
    ...
    if multiprocessing_style == 'many':
        ...
        for c in range(0,len(files),chunk_size):
            print("CHUNK",c,chunk_size)
            pool = multiprocessing.Pool(processes=16,maxtasksperchild=1)
            res = pool.imap(create_tabfile, zip(files[c:c+chunk_size],itertools.repeat(path,chunk_size),itertools.repeat(lang,chunk_size)))
            res = list[res]
            pool.close()

...
def test_quote_tib(quote):
    ...
    if not add_flag:
        ...
        if len(stripped_tokens) > 7 and len(tokens) > TIBETAN_MIN_LENGTH:
            half_add_flag = True
        if len(tokens) < 7:
            half_add_flag = False
        if half_add_flag:
...
def test_quote_skt(quote):
    quote_string = remove_punc(quote['par_string'])
    root_string = remove_punc(quote['root_string'])
    quote_ratio = get_chn_stopword_ratio(quote_string)
    if not "mavt" in quote['par_segnr'][0] and not "psk" in quote['par_segnr'][0]:
        if len(quote_string) > 20 and len(root_string) > 20:
            return True



def postprocess_quotes_skt(quotes):        
    result_quotes = []
    lost_count = 0 
    for quote in quotes:
        if test_quote_skt(quote):

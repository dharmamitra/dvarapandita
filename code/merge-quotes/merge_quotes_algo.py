...
def extend_quote(quote,current_quote,windowsize):
    ...
    if current_quote['quote_position_beg'] < quote['quote_position_beg']:
        quote['quote_offset_beg'] = current_quote['quote_offset_beg'] 
    ...
    if current_quote['head_position_beg'] < quote['head_position_beg']:
        quote['head_offset_beg'] = current_quote['head_offset_beg'] 
...
def merge_quotes_per_file_batch(quotes,windowsize):
    ...
    for quote in quotes:
        ...
        quotes_by_pair[tuple(pair)] = copy.deepcopy(quote)
...
def merge_quotes(quotes,windowsize):
    ...
    for current_file in list(quotes.keys()):
        ...
        #if 1==1:
        quote_buckets.append(quotes[current_file]) # comment this out when running single core
        #quote_results.append(merge_quotes_per_file([quotes[current_file],windowsize])) # comment this out when running multicore
...
def get_data_from_quote(in_quote,windowsize):
    ...
    return {
        ...
        "head_offset_beg":quote['head_offset_beg'],
        "head_offset_end":quote['head_offset_end'],
        "quote_offset_beg":quote['quote_offset_beg'],
        "quote_offset_end":quote['quote_offset_end'],        
